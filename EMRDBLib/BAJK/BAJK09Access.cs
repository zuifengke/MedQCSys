...
namespace EMRDBLib
{
    ...
    public class BAJK09Access : DBAccessBase
    {
        public const string TableName = SystemData.DataTable_BAJK.BAJK09;
        private static BAJK09Access m_Instance = null;

        /// <summary>
        /// 获取系统运行上下文实例
        /// </summary>
        public static BAJK09Access Instance
        {
            get
            {
                if (m_Instance == null)
                    m_Instance = new BAJK09Access();
                return m_Instance;
            }
        }

        public short GetBAJK09s(decimal brxh, ref List<BAJK09> lstBAJK09s)
        {
            ...
            szCondition = string.Format("{0} AND {1} = {2} "
                , szCondition
                , SystemData.BAJK09Table.KEY0901
                , brxh);
            ...
            try
            {
                ...
                if (lstBAJK09s == null)
                    lstBAJK09s = new List<BAJK09>();
                do
                {
                    BAJK09 bajk09 = new BAJK09();
                    for (int i = 0; i < dataReader.FieldCount; i++)
                    {
                        if (dataReader.IsDBNull(i))
                            continue;
                        PropertyInfo property = Reflect.GetPropertyInfo(typeof(BAJK09), dataReader.GetName(i));
                        bool result = Reflect.SetPropertyValue(bajk09, property, dataReader.GetValue(i));
                    }
                    lstBAJK09s.Add(bajk09);
        ...
        public short Insert(BAJK09 model)
        {
            if (model == null)
            {
            ...
            if (model.KEY0901 == 0 || model.KEY0902 == 0 || model.KEY0903 == 0)
            {
                return SystemData.ReturnValue.EXCEPTION;
            }
            StringBuilder sbField = new StringBuilder();
            StringBuilder sbValue = new StringBuilder();
            PropertyInfo[] PropertyList = Reflect.GetProperties<BAJK09>(model);
            ...
            string szSQL = string.Format(SystemData.SQL.INSERT
                , SystemData.DataTable_BAJK.BAJK09
                , szField
                , szValue);
            int nCount = 0;
            try
        ...
        public short Delete(decimal brxh)
        {
            ...
            string szCondition = string.Format("{0}={1}", SystemData.BAJK09Table.KEY0901, brxh);
