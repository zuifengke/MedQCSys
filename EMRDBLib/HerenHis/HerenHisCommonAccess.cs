...
namespace EMRDBLib
{
    public class HerenHisCommonAccess : DBAccessBase
    {
        private static HerenHisCommonAccess m_Instance = null;

        /// <summary>
        /// 获取系统运行上下文实例
        /// </summary>
        public static HerenHisCommonAccess Instance
        {
            get
            {
                if (m_Instance == null)
                    m_Instance = new HerenHisCommonAccess();
                return m_Instance;
            }
        }
        ...
        public short ExecuteQuery(string sql, out DataSet result)
        {
            ...
            try
            {
                result = base.HerenHisAccess.ExecuteDataSet(sql, CommandType.Text);
            }
            catch (Exception ex)
            {
