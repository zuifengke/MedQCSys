...
{
    /// <summary>
    /// 缺陷内容：缺出院记录或未在患者出院后24小时内完成
    /// 核查方法：患者病历出院24小时后无名称为“出院记录”的病历文件，患者“出院记录”病历文件的医生审签时间在患者出院24小时后。
    /// </summary>
    public class chuyuanjilushixiaoCommand : AbstractCommand
    {
        public chuyuanjilushixiaoCommand()
        {
            this.m_name = "新军卫-缺出院记录或未在患者出院后24小时内完成";
        }
        public override bool Execute(object param, object data, out object result)
        {
            ...
            if (documentlist == null || documentlist.Count == 0)
            {
                ...
                {
                    qcCheckResult.QC_EXPLAIN = "出院记录期限未到";
                    qcCheckResult.QC_RESULT = 1;
                    return true;
                }
                qcCheckResult.QC_EXPLAIN = "期限已到,未写出院记录";
            ...
            {
                qcCheckResult.QC_RESULT = 0;
                qcCheckResult.QC_EXPLAIN = string.Format("患者{0}出院，书写{1}时间{2},截止时间{3},结果超时"
            ...
            qcCheckResult.QC_RESULT = 1;
            qcCheckResult.QC_EXPLAIN = string.Format("患者{0}出院，书写{1}时间{2},截止时间{3},结果正常"
               , dtStartTime.ToString("yyyy-MM-dd HH:mm")
                    , documentlist[0].DOC_TITLE
                    , documentlist[0].DOC_TIME.ToString()
                    , dtEndTime.ToString("yyyy-MM-dd HH:mm")
