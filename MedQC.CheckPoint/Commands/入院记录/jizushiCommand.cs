...
{
    /// <summary>
    /// 缺陷内容：家族中有死亡者，死因未描述；或未记录父母情况
    /// 核查方法：入院记录家族史中，包含“父”或“母”两个关键词。50岁以上可以不检测。必须有“遗传”关键词。
    /// </summary>
    public class jiazushiCommand : AbstractCommand
    {
        public jiazushiCommand()
        {
            this.m_name = "新军卫-家族史";
        }
        public override bool Execute(object param, object data, out object result)
        {
            ...
            XmlNode node = doc.SelectSingleNode(string.Format("//Field[@Name='{0}']", "家族史"));
            if (node == null || string.IsNullOrEmpty(node.InnerText))//存在一项为空则直接返回不继续判断
            {
                qcCheckResult.QC_EXPLAIN = string.Format("{0}为空", "家族史");
                qcCheckResult.QC_RESULT = 0;
                qcCheckResult.ERROR_COUNT = 1;
                return true;
            }

            if (node.InnerText.IndexOf("父") < 0 && node.InnerText.IndexOf("母") < 0)
            {

                qcCheckResult.QC_EXPLAIN = string.Format("{0}未记录父母情况", "家族史");
                qcCheckResult.QC_RESULT = 0;
                qcCheckResult.ERROR_COUNT = 1;
                return true;
            }
            if (node.InnerText.IndexOf("遗传") < 0)
            {
                qcCheckResult.QC_EXPLAIN = string.Format("{0}未记录遗传病情况", "家族史");
                qcCheckResult.QC_RESULT = 0;
                qcCheckResult.ERROR_COUNT = 1;
                return true;
            }
