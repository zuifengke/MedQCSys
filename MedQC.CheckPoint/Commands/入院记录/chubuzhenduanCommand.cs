...
namespace Heren.MedQC.CheckPoint.Commands.newhis
{
    ...
    /// 核查方法：入院记录中初步诊断不能为空。
    /// </summary>
    public class chubuzhenduanCommand : AbstractCommand
    {
        public chubuzhenduanCommand()
        {
            this.m_name = "新军卫-无初步诊断";
        }
        public override bool Execute(object param, object data, out object result)
        {
            ...
            XmlNode node = doc.SelectSingleNode(string.Format("//Field[@Name='{0}']", "初步诊断"));
            if (node == null || string.IsNullOrEmpty(node.InnerText))//存在一项为空则直接返回不继续判断
            {
                qcCheckResult.QC_EXPLAIN = string.Format("{0}为空，规则不通过", "初步诊断");
