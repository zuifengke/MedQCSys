...
namespace Heren.MedQC.CheckPoint.Commands.newhis
{
    /// <summary>
    /// 缺陷点：缺术后第一天病程记录
    /// 核查内容：中等以上手术，术后第一天须有一个病程记录。如果患者术后第二天出院，第三天不进行核查。如手术结束时间在22点以后，手术结束后五小时内记录的病程记录算为第一天的病程记录，从结束后五小时到第二天23:59之前的病历算为第二天的病程记录。全部病程类记录有一即可。
    /// </summary>
    public class shouhoudiyitianbingchengshixiaoCommand : AbstractCommand
    {
        public shouhoudiyitianbingchengshixiaoCommand()
        {
            this.m_name = "新军卫-缺术后第一天病程记录";
        }
        public override bool Execute(object param, object data, out object result)
        {
            ...
            {
                result = null;
                LogManager.Instance.WriteLog("新军卫-缺术后第一天病程记录命令发生异常", ex);
                return false;
            }
        }
    }
